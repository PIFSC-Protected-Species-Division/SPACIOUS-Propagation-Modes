...
# ------------------------------------------------------------------
# Variable angular resolution  (fine bearings near, coarser far)
# ------------------------------------------------------------------
def bearing_step(max_range_km,
                 min_step_deg=1.0,      # finest spacing
                 max_step_deg=5.0,      # coarsest spacing
                 pivot_km=10.0,         # keep ≤ min_step until here
                 limit_km=40.0):        # reach max_step at this range
    """
    Return an angular spacing (degrees) that grows linearly with the
    ray length you are going to model.

    * ≤ pivot_km → min_step_deg
    * ≥ limit_km → max_step_deg
    * between    → linear interpolation
    """
    if max_range_km <= pivot_km:
        return min_step_deg
    if max_range_km >= limit_km:
        return max_step_deg
    frac = (max_range_km - pivot_km) / (limit_km - pivot_km)
    return min_step_deg + frac * (max_step_deg - min_step_deg)


def build_bearing_array(max_range_km,
                        min_step_deg=1.0,
                        max_step_deg=5.0):
    """Return a 1-D numpy array of bearings 0…360 ° with variable spacing."""
    step = bearing_step(max_range_km, min_step_deg, max_step_deg)
    # np.arange stops before 360 so we append it explicitly
    bearings = np.arange(0.0, 360.0, step, dtype=np.float32)
    if bearings[-1] < 360.0 - 1e-6:
        bearings = np.append(bearings, 360.0)
    return bearings[:-1]          # drop the duplicate 360
def variable_bearings(
    min_spacing_deg=1.0,
    max_spacing_deg=5.0,
    near_dist_km=5.0,
    far_dist_km=40.0,
    freq_khz=10.0):
    """
    Compute variable bearing angles based on desired angular resolution.

    min_spacing_deg: fine spacing near the source (e.g., 1°)
    max_spacing_deg: coarse spacing at far distances (e.g., 5°)
    near_dist_km: distance at which fine spacing is needed
    far_dist_km: distance where coarse spacing is acceptable
    freq_khz: frequency in kHz, used to estimate max detection range

    Returns:
        np.ndarray of angles in degrees
    """
    if freq_khz < 20:
        max_distance_km = (45 - freq_khz)  # from your logic
    else:
        max_distance_km = (35 - freq_khz)

    num_segments = int(360 / min_spacing_deg)
    bearings = [0]

    while bearings[-1] < 360 - min_spacing_deg:
        r = bearings[-1]
        # linearly interpolate spacing between min and max
        dist_frac = r / 360
        spacing = min_spacing_deg + dist_frac * (max_spacing_deg - min_spacing_deg)
        bearings.append(min(360, r + spacing))

    return np.array(bearings[:-1])  # drop final 360 if it exists
...
def calcTL(subset_df, drifter_lat, drifter_lon, freq_hz, bearing_deg,
    ...
    env['rx_range'] = np.arange(0, bathy_grid['range'].iloc[-1], 200)
        ...
        if np.max(profile['depth'])>200:
            ...
            for freq in freq_hz:
                ...
                if freq < 20000:
                    ...
                    max_distance_km= (35000-freq)/1000
                ...
                bearings_deg = build_bearing_array(max_distance_km)   # <= new helper
                print(f"  » modelling {len(bearings_deg)} rays "
                      f"({np.min(np.diff(bearings_deg)):.1f}°-{np.max(np.diff(bearings_deg)):.1f}° spacing)")
                
                tasks = [
                    (float(angle), drifter_lat, drifter_lon, freq, ssp, txDepth,
                     max_distance_km, interval, hydVertSpacing)
                    for angle in bearings_deg]
                ...
                with ThreadPool(processes=N_POOL) as pool:
                    for status, ii, payload in pool.imap_unordered(_safe_worker,
                        ...
                        print(f"Processed {ii} of {len(bearings_deg)} rays – {freq/1000:.1f} kHz, dive {driftId}")
                ...
                save_dive_frequency(
                 h5_path      = "Spacious_Hawaii_100m_rays_v2.h5",
                 drift_id     = "01",
                 dive_id      = driftId,
                 freq_khz     = freq,
                 metadata     = metadata,
