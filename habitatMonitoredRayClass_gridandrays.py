...
def adaptive_angular_sampling(
    max_range_km: float,
    base_step_deg: float = 5.0,
    falloff_km: float = 10.0,
    min_step_deg: float = 0.5) -> np.ndarray:
    """
    Generate non-uniform bearing angles for ray tracing that increase
    in angular resolution with distance from the source.
    
    Parameters
    ----------
    max_range_km : float
        The maximum propagation distance in kilometers.
    base_step_deg : float
        The widest angular step (typically near the source).
    falloff_km : float
        The distance over which the angular step refines.
    min_step_deg : float
        The finest angular step allowed at long range.
    
    Returns
    -------
    np.ndarray
        Array of cumulative angle values in degrees, from 0 to ≤ 360.
    """
    ranges_km = np.linspace(0.1, max_range_km, num=200)
    steps_deg = base_step_deg / (1 + ranges_km / falloff_km)
    steps_deg = np.clip(steps_deg, min_step_deg, base_step_deg)
    cumulative_angles = np.cumsum(steps_deg)
    return cumulative_angles[cumulative_angles <= 360.0]
...
class PropagationModelRunner:
    ...
    def run(self, drift_ids: Iterable[str] | None = None) -> None:
        """
        Crunch each dive and append its TL grids to `self.h5_out`, using adaptive angular sampling.
        """
        drift_ids = (
            list(drift_ids)
        ...
        for dive_id in drift_ids:
            ...
            if self.fixedDrifterDetph is not None:
                drifter_depth = self.fixedDrifterDetph
            else:
                drifter_depth = group['Depth_m'][group['Depth_m'].first_valid_index()]
            ...
            metadata = dict(start_lat=lat0, start_lon=lon0, drifter_depth=drifter_depth)

            for f_hz in self.freq_khz:

                max_range_km = (45_000 - f_hz) / 1000 if self.freq_dep_rad else self.bathy_radius_km

                angles = adaptive_angular_sampling(
                ...
                tasks = [
                    (ang, lat0, lon0, f_hz, ssp_table,
                     drifter_depth, max_range_km,
                     self.bathy_interval_m, self.hyd_vert_m)
                    for ang in angles
                ]
                ...
                with ThreadPool(processes=self.cores) as pool:
                    for status, ang, payload in pool.imap_unordered(
                        ...
                        if status == "fail":
                            print(f"❌  bearing {ang:>3.1f}°  →  {payload!r}")
                            continue
                        results.append(
                            dict(
                                angle_deg=payload[0],
...
if __name__ == "__main__":
    ...
    runner = PropagationModelRunner(
        gebco_nc=nc_file_loc,
        drift_csv=drift_csv_loc,
        h5_out='adaptive_sampled_rays.h5',
        cores=4,
        freq_khz=(10000,),
        bathy_radius_km=5,
        n_bearings=360,             # ← This is ignored now, kept for compatibility
        hyd_vert_m=50,
        bathy_interval_m=100,
    )
    
    runner.run()  # <-- This now uses adaptive angular sampling under the hood
