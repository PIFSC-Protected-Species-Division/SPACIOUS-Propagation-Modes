# -*- coding: utf-8 -*-
"""
Bellhop ray-fan driver – process-parallel, Windows-safe
Created Apr 2025 • revised Apr 26 2025
Author: Kaity (with a little help)
"""
# ------------------------------------------------------------ 0. housekeeping
import os
# 4 OpenBLAS threads × N workers  ≤ 56  (c2d-standard-56)
...
# the big objects below are filled by _init_worker()
subset_df        = None
ssp              = None
drifter_lat      = None
drifter_lon      = None
...
# ------------------------------------------------------------ 2. helper funcs
# … (all your helper functions unchanged: variable_rx_range, save_dive_frequency,
#     haversine, calculate_initial_compass_bearing, interpolate_sound_speed,
#     checkEnv, extract_bathymetry_along_ray_vectorized)
#     I kept them exactly as you gave them – omitted here for brevity
# ---------------------------------------------------------------------------
...
# ------------------------------------------------------------ 3. pool helpers
def _init_worker(init_subset_df, init_ssp,
                 init_drifter_lat, init_drifter_lon,
                 init_freq_hz, init_tx_depth,
                 init_max_dist_km, init_interval_m,
                 init_hyd_vert_spacing):
    """
    Runs ONCE in every child process.
    Broadcasts large objects via globals and caps BLAS threads.
    """
    global subset_df, ssp, drifter_lat, drifter_lon
    global freq_hz, txDepth, max_distance_km, interval_m, hydVertSpacing

    subset_df        = init_subset_df
    ssp              = init_ssp
    drifter_lat      = init_drifter_lat
    drifter_lon      = init_drifter_lon
    freq_hz          = init_freq_hz
    txDepth          = init_tx_depth
    max_distance_km  = init_max_dist_km
    interval_m       = init_interval_m
    hydVertSpacing   = init_hyd_vert_spacing

    # every child gets its own 4-thread OpenBLAS
    os.environ["OPENBLAS_NUM_THREADS"] = "4"
    os.environ["OMP_NUM_THREADS"]      = "4"


# ### FIX: _worker now takes ONE scalar (angle) – all else is global
def _worker(angle_deg: int):
    """
    Do one Bellhop ray and return TL grid + axes.
    """
    bath_vals, lons, lats, r_km = extract_bathymetry_along_ray_vectorized(
        subset_df       = subset_df,
        start_lat       = drifter_lat,
    ...
    bathy_grid.drop_duplicates(inplace=True)
    bathy_grid.sort_values("range", inplace=True)
    bathy_grid.loc[0, "range"] = 0.0

    # trim shallow segment
    shallow = np.where(bathy_grid["depth_m"] < 100)[0]
    if shallow.size:
        bathy_grid = bathy_grid.iloc[: shallow.min()]

    bathy = bathy_grid.apply(
        lambda row: [row["range"], row["depth_m"]], axis=1).tolist()
    ...
    env = pm.create_env2d(
        ...
        tx_depth=txDepth,
        frequency=freq_hz,
        nbeams=0, max_angle=90, min_angle=-90)

    env["rx_range"] = variable_rx_range(bathy_grid["range"].iat[-1] / 1000) * 1000
    env["rx_depth"] = np.arange(0, bathy_grid["depth_m"].max(), hydVertSpacing)

    tl_start = time.time()
    tloss = pm.compute_transmission_loss(env, mode="coherent")
    print(f"ray {angle_deg:3d} done in {time.time() - tl_start:5.1f} s")

    tlos_db = 20.0 * np.log10(np.abs(tloss) + 0.01)

    # mask below seabed
    seabed_neg = np.interp(env["rx_range"],
                           bathy_grid["range"].to_numpy(),
                           bathy_grid["depth_m"].to_numpy(),
                           left=np.nan, right=np.nan)
    mask = env["rx_depth"][:, None] > seabed_neg[None, :]
    mask[:, 0] = True
    tloss_masked = np.where(mask, np.nan, tlos_db)

    return angle_deg, tloss_masked, env["rx_range"], env["rx_depth"]


def _safe_worker(angle_deg):
    """Wrap _worker so a failure doesn’t kill the pool."""
    try:
        return ("ok",) + _worker(angle_deg)
    except Exception as exc:
        traceback.print_exc()
        return ("fail", angle_deg, exc)

# ------------------------------------------------------------ 4. main driver
if __name__ == "__main__":

    # ---------------- hardware budget
    vcpus     = mp.cpu_count()          # 56 on c2d-standard-56
    nWorkers  = max(vcpus - 2, 4)       # keep 2 cores free
    print(f"using {nWorkers} worker processes, 4 BLAS threads each")

    # ---------------- load glider & bathy
    driftCTD = pd.read_csv(r"C:\Users\pam_user\Documents\GitHub\SPACIOUS-Propagation-Modes\modelling\sg639_MHI_Apr2023_CTD.csv")
    nc_file  = r"C:\Users\pam_user\Documents\GitHub\SPACIOUS-Propagation-Modes\bathymetry\GEBCO_28_Mar_2025_ade9db365e34\gebco_2024_n23.5_s18.5_w-160.0_e-154.0.nc"
    ds       = xr.open_dataset(nc_file)
    lon_mesh, lat_mesh = np.meshgrid(ds["lon"].values, ds["lat"].values)
    bathy_full = pd.DataFrame({
        "depth": ds["elevation"].values.ravel(),
        "lat":   lat_mesh.ravel(),
        "lon":   lon_mesh.ravel()})
    ...
    driftCTD.loc[0, "Direction"] = "dec" if depth_diff[1] > 0 else "asc"
    driftCTD["DiveID"]       = driftCTD["DiveNumber"].astype(str) + "_" + driftCTD["Direction"]
    unique_ids               = driftCTD["DiveID"].unique()

    # ------------------------------------------------ loop over dives
    for driftId in unique_ids[15:]:
        group = driftCTD[driftCTD["DiveID"] == driftId]
        drifter_lat, drifter_lon = group[["Latitude", "Longitude"]].iloc[0]

        # pre-filter bathy within 40 km
        bathy_full["distance_km"] = haversine(
            drifter_lon, drifter_lat, bathy_full["lon"], bathy_full["lat"])
        subset_df = bathy_full[bathy_full["distance_km"] <= max_distance_km]

        # build SSP list [[depth, ss] …]
        prof = (group[["Depth_m", "SoundSpeed_m_s"]]
                .rename(columns={"Depth_m": "depth", "SoundSpeed_m_s": "ss"})
                .dropna()
                .sort_values("depth")
                .reset_index(drop=True))
        prof.loc[0, "depth"] = 0
        if prof["depth"].max() < 200:
            print(f"skip {driftId}: profile too shallow")
            continue
        last_ss = prof["ss"].iat[-1]
        padded  = pd.DataFrame({
            "depth": np.arange(prof["depth"].iat[-1] + 10,
                               subset_df["depth"].abs().max() + 50, 50),
            "ss":    last_ss})
        ssp = pd.concat([prof, padded]).apply(lambda r: [r["depth"], abs(r["ss"])], axis=1).tolist()
        ...
        print(f"▶️  {driftId}: launching 360 rays …")
        results = {driftId: []}
        ...
        with ProcessPoolExecutor(
            ...
            futures = {pool.submit(_safe_worker, ang): ang for ang in range(360)}
        
            completed = 0
            for fut in as_completed(futures):
                status, ang, payload = fut.result()
        
                if status == "fail":
                    print(f"❌  ray {ang}: {payload}")
                    continue               # skip failed ray
        
                angle_deg, tloss_masked, rx_range, rx_depth = payload
                results[driftId].append({
                    "angle_deg": angle_deg,
                    "range_m":   rx_range,
                    "depth_m":   rx_depth,
                    "transmission_loss": tloss_masked
                })
        
                completed += 1
                print(f"✓ processed {completed:3d} / 360 rays", flush=True)
